        ...
        const RT = {
            priceMs: 200,       // 5 UI price updates/sec
            orderbookMs: 300,   // ~3.3 orderbook recalculations/sec
            overlaysMs: 2000,   // S/R + pattern recalculation every 2 sec
            indicatorsMs: 2000, // RSI/MACD every 2 sec
            lastPricePaint: 0,
        ...
        function initChart() {
            ...
            ema20Series = chart.addLineSeries({ color: '#f7a600', lineWidth: 1.4, title: 'EMA 20', lastValueVisible: false, priceLineVisible: false });
            ema50Series = chart.addLineSeries({ color: '#29b6f6', lineWidth: 1.4, title: 'EMA 50', lastValueVisible: false, priceLineVisible: false });
            ema200Series = chart.addLineSeries({ color: '#ab47bc', lineWidth: 1.6, title: 'EMA 200', lastValueVisible: false, priceLineVisible: false });
            ...
            rsi50Series = rsiChart.addLineSeries({
        ...
        function renderPosition(pos) {
            ...
            if (has) {
                ...
                set('mobilePosAvg', formatPrice(pos.avgPrice));
                set('mobilePosMark', formatPrice(pos.markPrice || livePrice || 0));
            ...
            if (has) {
                ...
                const riskMeta = `${tp ? ` · TP ${formatPrice(tp)}` : ''}${sl ? ` · SL ${formatPrice(sl)}` : ''}`;
                text = `${sideLabel} ${pos.size} @ ${formatPrice(pos.avgPrice)} · ${pnl >= 0 ? '+' : ''}${pnl.toFixed(3)} USDT${riskMeta}`;
        ...
        async function executeTrade(side) {
            ...
            try {
                ...
                if (multiActive) {
                    ...
                    const pr = tradeState.position;
                    if (pr) {
                        const sr = await fetch('/api/trading-stop', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({symbol:currentSymbol, positionIdx:Number(pr.positionIdx||0), takeProfit:tp||undefined, stopLoss:sl||undefined, tpTriggerBy:'MarkPrice', slTriggerBy:'MarkPrice'})});
                        const sj = await sr.json(); if(!sr.ok||!sj.ok) throw new Error('Позиция открыта, но TP/SL не установлен: '+(sj.error||''));
                        setTradeStatus('Позиция открыта, TP/SL установлен', 'ok');
                    } else if (orderType === 'Limit') {
                        setTradeStatus('Limit принят. TP/SL установится после открытия позиции.', 'ok');
                    } else {
                        setTradeStatus('Ордер принят, но позиция ещё не появилась. TP/SL не установлен.', 'error');
                    }
                }
            } catch(err) { setTradeStatus(err.message, 'error'); }
        ...
        function bindTradingUI() {
            ...
            const savedCollapsed = localStorage.getItem('terminal.mobileTradeCollapsed') === '1';
            ...
            updateLimitVisibility(); loadTradeConfig(); refreshPosition(); setPreviewSide('Buy'); ensureDynamicRiskTicker(); renderDynamicRiskNow(); updateTradePreview();
        ...
        function processLiveOrderbook(force = false) {
            ...
            detectedWalls = [...processOrders(asks, 'sell'), ...processOrders(bids, 'buy')];
            drawOrderBookWalls();
            renderOrderbookWallsPanel();
        }
        ...
        function drawOrderBookWalls() {
            ...
            if (!settings.showOrderbook || !detectedWalls || detectedWalls.length === 0) return;

            detectedWalls.forEach(wall => {
                const isBuy = wall.type === 'buy';
                const formattedVol = formatVolumeUSD(wall.usdValue);
                const line = candleSeries.createPriceLine({
                    ...
                    title: `${isBuy ? '🟢 BW' : '🔴 SW'} (${formattedVol})`,
                });
        ...
        function scheduleRealtimePaint() {
            ...
            RT.raf = requestAnimationFrame(() => {
                ...
                if (RT.pendingPrice || RT.pendingKline || RT.pendingOrderbook || (rawCandles.length && ((RT.overlayDirty && now-RT.lastOverlaysPaint >= RT.overlaysMs) || (RT.indicatorDirty && now-RT.lastIndicatorsPaint >= RT.indicatorsMs)))) {
                    scheduleRealtimePaint();
                }
            });
        }
